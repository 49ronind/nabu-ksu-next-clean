...
def inject_hook(kernel_dir, relative_path, function_signature,
    ...
    if function_start is None:
        fail("Function signature not found in " + relative_path + ": " + function_signature)

    function_end = find_function_end(lines, function_start)
    if function_end is None:
        fail("Could not locate function end in " + relative_path + ": " + function_signature)

    function_text = "".join(lines[function_start:function_end + 1])
    if unique_marker in function_text:
        print("Already patched: " + relative_path + " -> " + unique_marker)
        return
    ...
    if anchor_index is None:
        fail("Anchor not found in " + relative_path + ", " + function_signature + ": " + anchor)
    ...
    if mode == "before":
        ...
        fail("Unknown injection mode: " + mode)

    path.write_text("".join(lines), encoding="utf-8")
    print("Injected: " + relative_path + " -> " + unique_marker)
